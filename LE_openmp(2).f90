program EspectroLyapunov
    ...
    ! Cada iteracion es independiente: problema embarazosamente paralelo
    !------------------------------------------------------------------
    !$omp parallel do                                                   &
...
!**********************************************************************
    ...
    subroutine calcular_exponentes(RR_loc, idx)
        ...
        do m = 1, Nr
            ...
            ! Cada hilo imprime su progreso cada IO pasos
            if (mod(m, IO) == 0) then
                print '(A,F6.3,A,I10,A,F10.5,A,F10.5,A,F10.5)', &
                    'r2=', RR_loc,                                  &
                    '  m=', m,                                      &
                    '  L1:', cum(1)/t,                              &
                    '  L2:', cum(2)/t,                              &
        ...
        !--------------------------------------------------------------
        ! Resumen final para este valor de r2
        !--------------------------------------------------------------
        print '(A,F6.3,A,F10.5,A,F10.5,A,F10.5,A,F10.5,A,F10.5,A,ES10.3)', &
            'FINAL r2=', RR_loc,                                               &
            '  L1:', cum(1)/t,                                                 &
            '  L2:', cum(2)/t,                                                 &
            '  L3:', cum(3)/t,                                                 &
...
!**********************************************************************
    pure function f12(r, t, RR_loc) result(res)
        ...
        J(2,1) = -d*v
        J(2,2) = RR_loc*(1._qp - 2._qp*v) - d*u   ! corregido
        J(2,3) = 0._qp
